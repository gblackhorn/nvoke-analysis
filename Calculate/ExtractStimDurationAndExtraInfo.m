function [StimDuration,varargout] = ExtractStimDurationAndExtraInfo(StimRange,varargin)
	...
	for stn = 1:stimtype_num % loop through every stimulation type
		...
		[StimDuration(stn)] = CalculateStimDuration(TimeRange,'round_digit_sig',round_digit_sig);
	...
	if ~ExtraInfo.multistim
		...
		[~, stim_end_order] = sort(stim_end_time,'decend'); % When various stim applied, use the one end late as the stim_end
		stim_end_first_loc = stim_end_order(1);

		UnifiedStimDuration.range = [StimDuration(stim_start_first_loc).range(:,1) StimDuration(stim_end_first_loc).range(:,2)];

		[UnifiedStimDuration] = CalculateStimDuration(UnifiedStimDuration.range,'round_digit_sig',round_digit_sig);
