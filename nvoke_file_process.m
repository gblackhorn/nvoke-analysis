...
for n = 1:length(dirinfo_data_dir_chosen)
	...
	for m = 1:length(raw_file_info)
		...
		if ~exist(mc_file, 'file')
			% % motion correction using mean image as reference
			% isx.motion_correct(bp_file, mc_file,...
			% 	'max_translation', 20, 'reference_file_name', mean_proj_file,...
			% 	'low_bandpass_cutoff', 0.004, 'high_bandpass_cutoff', 0.016,...
			% 	'output_translation_files', 'translation_files', 'output_crop_rect_file', 'crop_rect_file');

			% motion correction using 10th image as reference
			isx.motion_correct(bp_file, mc_file,...
				'max_translation', 20,...
				'reference_segment_index', 0, 'reference_frame_index', 10,...
				'low_bandpass_cutoff', 0.004, 'high_bandpass_cutoff', 0.016,...
				'output_translation_files', 'translation_files', 'output_crop_rect_file', 'crop_rect_file');
