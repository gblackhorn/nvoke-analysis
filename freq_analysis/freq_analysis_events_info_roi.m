function [event_info_struct,varargout] = freq_analysis_events_info_roi(peak_properties_table,stimulation_win,recording_time,varargin)
    ...
    % Main contents
    [empty_event_info_struct, varargout] = creat_event_struct;
    if isempty(stimulation_win)
        event_info_struct = empty_event_info_struct;
        ...
        for fn = fieldnames(event_info)'
           empty_event_info_struct(:).(fn{1}) = deal(event_info.(fn{1}));
        end
        event_info_struct = empty_event_info_struct;
    end
    varargout{1} = setting;
