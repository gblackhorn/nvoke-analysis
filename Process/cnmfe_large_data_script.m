...
%% parameters
% -------------------------    COMPUTATION    -------------------------  %
pars_envs = struct('memory_size_to_use', 54, ...   % default=8, GB, memory space you allow to use in MATLAB
    'memory_size_per_patch', 2, ...   % default=0.6, GB, space for loading data within one patch
    'patch_dims', [64, 64]);  % default=[64, 64], GB, patch size

% -------------------------      SPATIAL      -------------------------  %
gSig = 16;           % default=3, pixel, gaussian width of a gaussian kernel for filtering the data. 0 means no filtering
gSiz = 32;          % default=13, pixel, neuron diameter
...
merge_thr = 0.65;     % default=0.65 alternative=[1e-1, 0.85, 0], thresholds for merging neurons; [spatial overlap ratio, temporal correlation of calcium traces, spike correlation]
method_dist = 'max';   % method for computing neuron distances {'mean', 'max'}
dmin = 2;       % default=5, minimum distances between two neurons. it is used together with merge_thr
...
bd = 1;             % default=0, number of rows/columns to be ignored in the boundary (mainly for motion corrected data)
