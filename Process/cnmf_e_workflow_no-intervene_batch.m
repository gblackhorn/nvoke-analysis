...
default_init_folder = 'G:\Workspace\Inscopix_Seagate\Projects\Exported_tiff';
folder = uigetdir(default_init_folder,...
    'Select a folder containing to-be processed files in subfolders');
if folder == 0
    return
else
    default_init_folder = folder;
end
...
for i = 1:subfolders_num % Ignore "." and ".." 
    ...
    if isempty(cnmfe_result_file)
        tiff_files = dir(fullfile(subfolder, '*-mc*.tif*')); % list .tif and .tiff files
        tiff_files = tiff_files(~contains({tiff_files.name}, '-dff', 'IgnoreCase', true)); % discard deltaF/F file
        if length(tiff_files) > 1
            [~, idx] = sort([tiff_files.datenum]); % sort files according to date
            tiff_file = tiff_files(find(max(idx))); % use the latest modified file
...
neuron = Sources2D(); 
nams = file_names;          % you can put all file names into a cell array; when it's empty, manually select files 
nams = neuron.select_multiple_files(nams);  %if nam is [], then select data interactively 

%% parameters  
% -------------------------    COMPUTATION    -------------------------  %
pars_envs = struct('memory_size_to_use', 8, ...   % GB, default=8, memory space you allow to use in MATLAB 
    'memory_size_per_patch', 0.5, ...   % GB, default=0.5, space for loading data within one patch 
    'patch_dims', [64, 64],...  %GB, default=[64, 64], patch size 
    'batch_frames', 1000);           % default = 1000, number of frames per batch 
  % -------------------------      SPATIAL      -------------------------  %
