function [TransientProperties,varargout] = FindPeaksWithExistingPeakInfo(roi_trace,existing_peakInfo,varargin)
 ...
 %    TransientProperties_col_names = {'peak_loc', 'peak_mag', 'rise_loc', 'decay_loc','peak_time',...
    ...
    if ~isempty(existing_peakInfo)
    	...
    	[roi_trace_processed,filter_info] = process_roi_trace_with_filter(roi_trace,...
    				...
    				'recording_fq', rec_fq, 'time_info', time_info);
    	...
    	[peak_mag,peak_loc] = find_peaks_in_windows(roi_trace_window,window_start_time_index);

    	[transient_properties] = calculate_transient_properties(roi_trace_processed,...
    		time_info,peakMag,peakLoc,varargin)
