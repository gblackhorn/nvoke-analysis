function [f,varargout] = plot_TemporalData_Color_seperateStimRepeats(plotWhere,fluroData,timeData,stimInfo,varargin)
	...
	for ii = 1:2:(nargin-4)
        ...
        % elseif strcmpi('stimTypes', varargin{ii})
        %     stimTypes = varargin{ii+1}; 
    ...
    StimRangeIDX = NaN(size(stimRanges));
    [stimRanges(:,1),StimRangeIDX(:,1)] = find_closest_in_array(stimRanges(:,1),timeData);
    [stimRanges(:,2),StimRangeIDX(:,2)] = find_closest_in_array(stimRanges(:,2),timeData);
    ...
    plotRangeIDX(1,1) = StimRangeIDX(1,1); % start from the 1st stimulation 
    ...
    for sn = 2:stimNum
    	plotRangeIDX(sn,1) = StimRangeIDX(sn);
    	plotRangeIDX(sn,2) = StimRangeIDX(sn)+datapointNum-1;
    end
