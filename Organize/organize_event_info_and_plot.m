function [peak_info_sheet, varargout] = organize_event_info_and_plot(ROIdata, plot_analysis, triggeredPeak_filter)
...
if isstruct(ROIdata{1, 2})
	...
	peakinfo_row_name = 'peak_lowpass';
else
...
for rn = 1:recording_num
	...
	for roi_n = 1:roi_num
		if ~isempty(ROIdata{rn,5}{peakinfo_row_name, roi_n}{:})
			...
			roi_peaksize = size(ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('rise_time')); % number of peaks in one roi
			...
			peak_start = ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('rise_time'); % time points of peak start
			peak_end = ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('decay_time');
			rise_duration = ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('rise_duration');
			decay_duration = ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('decay_duration');
			peak_amp = ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('peak_mag_delta'); % relative one
			peak_slope = ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('peak_slope'); % peak slope
			peakslope_normhp = ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('peak_slope_norm_hpstd'); % peak slope
			...
			peak_relative_norm_hp = ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('peak_delta_norm_hpstd'); % peak normalized to std of highpassed data
			% riseTime_stimStart = ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('riseTime2stimStart');

			Exist_Column_peakCategory = strcmp('peak_category',ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.Properties.VariableNames);
			...
			if val_peakCategory_exist == 1
				peak_category = ROIdata{rn,5}{peakinfo_row_name, roi_n}{1, 1}.('peak_category'); % strings: [stim_str, '-triggered'] or not-triggered, rebound, etc.
...
if isempty(riseDuration)
	...
	peak_info_sheet = table(recNo, roiNo, recName, roiName, peakStart,...
		...
		peakSlope, peakSlope_normhp, peakNormHP, peakStim, peakCategory);
	...
	elseif triggeredPeak_filter == 2
		...
		ind_cell = strcmp('noStimFar', peakCategory);
		ind_row_discard = find(ind_cell==0);
		peak_info_sheet(ind_row_discard, :) = [];

		% discard_peaks = find(peakCategory==0);
		triggeredPeak_filter_string = 'noStimFar';
		nostim = true;
		% peak_info_sheet(discard_peaks, :) = [];
	...
	if size(peak_info_sheet.riseDuration, 1) <= 2
		...
		if nargin >= 2
			if plot_analysis == 1 || plot_analysis == 2 
				...
				str_fn = [char(peak_stim{1}), '-', triggeredPeak_filter_string];
