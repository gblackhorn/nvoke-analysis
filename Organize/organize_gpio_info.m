	    ...
	    for cn = 1:stim_ch_num % loop through gpio channels used for stimulation
			...
			if ~isempty(gpio_rise_loc)
				...
				if contains(stim_ch_name{cn}, 'GPIO-1') % airpuff trigger signal is 0.5s/1s. stimulation is 100ms
					...
					if ~isfield(gpio_info,'durations') || isempty(gpio_info(stim_ch_idx).durations)
						% gpio_info(stim_ch_idx).durations = stim_durations;
						gpio_info(stim_ch_idx).durations = round(stim_durations,round_digit_sig,'significant');;
					end
