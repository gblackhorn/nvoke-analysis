function nameAndTagStruct = createLocTagStruct(neuronNames,useSameTags,varargin)
    ...
    if useSameTags
       ...
       promptMsgTag1 = sprintf('Enter the tag for the ROIs with coordinations SMALLER than %d on %s axis (origin is top-left) : ',...
        userCoorVal,coorAxis);
       promptMsgTag2 = sprintf('Enter the tag for the ROIs with coordinations LARGER than %d on %s axis  (origin is top-left): ',...
        userCoorVal,coorAxis);
