function [peak_properties_tables_with_cat,varargout] = organize_category_peaks_multirois(peak_properties_tables,gpio_info_table,varargin)
    ...
    for rn = 1:roi_num
    	...
    	stim_ch_num = size(gpio_info_table, 1) > 1
    	if ~isempty(gpio_info_table)
	    	...
	    	for sn = 1:stim_ch_num
	    		[peak_category(:, sn)] = organize_category_peaks(peak_properties_table,...
	    ...
	    peak_properties_tables_with_cat{1, rn}{:} = [peak_properties_table_single, peak_category];
    end
end
