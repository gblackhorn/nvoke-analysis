function addPval2StatSummaryLatexTab(folderPath, file1Keyword, file2Keyword, optionalKeyword)
    ...
    %
    % Example:
    %   addPval2StatSummaryLatexTab('path/to/files', 'meanSemTab nNumInfo', ...
    %                               'modelCompTab', 'normalizedAmp');
    ...
    for i = 1:length(pairedFiles)
        ...
        % Initialize new table content with the modified header and data rows
        newTableContent = {header};
        ...
        for j = 1:length(tabRows)
            ...
            % If it's the last row, add pValue and hValue
            if j == length(tabRows)
        ...
        updatedFileName = fullfile(folderPath, ['updated_' meanSemTabFile]);
