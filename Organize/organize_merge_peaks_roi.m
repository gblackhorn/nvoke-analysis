function [peak_properties_table_updated,varargout] = organize_merge_peaks_roi(peak_properties_table,time_info,roi_trace,varargin)
    ...
    % Assign the input, peak_properties_table, to the output. Modification will be applied to the
    % output 
    peak_properties_table_updated = peak_properties_table;

    % Get the magnitude and time info of peaks
    ...
    % Merge two peaks if they are too close, and the following peak is bigger than the one before
    % it. Use interval setting to decide if they are "too close"
