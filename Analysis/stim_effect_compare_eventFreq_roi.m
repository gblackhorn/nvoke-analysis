function [logRatio,varargout] = stim_effect_compare_eventFreq_roi(events_time,stim_range,timeDuration,varargin)
	% Return the logarithm of event frequencies (inside_stim/outside_stim) in a single ROI
	...
	if isempty(sponEvent)
		sponEventNum = 0.00001; % assign a small number if there is no spontaneous event
	else
		sponEventNum = numel(sponEvent);
	end
	if isempty(stimEvent)
		stimEventNum = 0.00001; % assign a small number if there is no spontaneous event
	else
