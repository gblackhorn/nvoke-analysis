function [me, varargout] = mixed_model_analysis(dataStruct, responseVar, groupVar, hierarchicalVars, varargin)
    ...
    if ~isempty(tbl) && numel(MMdata.(responseVar)) > 1
        ...
        if ~isempty(binVar) && any(fixedEffectsStats.pValue < 0.05)
            [multiComparisonResults, mmPvalue] = performPostHocBinComparisons(me, groupLevels, binLevels, dispStat, modelType);
        end
...
function [results, mmPvalue] = performPostHocBinComparisons(me, groupLevels, binLevels, dispStat, modelType)
    % Extract the fixed effects and their standard errors
    [fixedEffectsEstimates, ~, fixedEffectsSE] = fixedEffects(me);
    SEs = fixedEffectsSE.SE;

    % Number of groups and bins
    numGroups = length(groupLevels);
    numBins = length(binLevels);

    % Initialize results storage
    results = [];
    mmPvalue = struct('method', {}, 'group1', {}, 'group2', {}, 'bin', {}, 'p', {}, 'h', {});

    % Calculate total number of comparisons for Bonferroni correction
    totalComparisons = numBins * (numGroups - 1); % Only comparing PO to DAO, which is the reference
    alpha = 0.05; % Desired significance level
    correctedAlpha = alpha / totalComparisons;

    % Perform pairwise comparisons within each bin
    for b = 1:numBins
        bin = binLevels{b};
        
        % Convert bin label to numerical index if necessary
        binIndex = str2double(bin); 

        % Calculate the estimated effect for each group within the current bin
        % For DAO (reference category)
        estimateDAO = fixedEffectsEstimates(1) + fixedEffectsEstimates(3) * binIndex;
        SE_DAO = sqrt(SEs(1)^2 + (binIndex * SEs(3))^2);

        % For PO
        estimatePO = fixedEffectsEstimates(1) + fixedEffectsEstimates(2) + ...
                     fixedEffectsEstimates(3) * binIndex + fixedEffectsEstimates(4) * binIndex;
        SE_PO = sqrt(SEs(1)^2 + SEs(2)^2 + (binIndex * SEs(3))^2 + (binIndex * SEs(4))^2);

        % Calculate the difference in estimates and its standard error
        estimateDiff = estimatePO - estimateDAO;
        seDiff = sqrt(SE_DAO^2 + SE_PO^2);

        % Calculate confidence intervals and p-values
        tValue = estimateDiff / seDiff;
        df = me.DFE;
        pValue = 2 * (1 - tcdf(abs(tValue), df));
        hValue = pValue < correctedAlpha; % Adjusted significance level

        % Store the results
        results = [results; 1, 2, b, estimateDiff, seDiff, tValue, df, pValue, hValue];

        % Append results to mmPvalue
        mmPvalue(end+1) = struct('method', modelType, 'group1', groupLevels{1}, 'group2', groupLevels{2}, 'bin', bin, 'p', pValue, 'h', hValue); %#ok<AGROW>
    end

    % Optionally display pairwise comparison results
    if dispStat
        fprintf('\nPairwise Comparisons by Bin (Bonferroni corrected):\n');
        for i = 1:size(results, 1)
...
%     % Extract main effects and interaction terms
%     intercept = fixedEffectsEstimates(1);
%     groupEffect = fixedEffectsEstimates(2);
%     binEffect = fixedEffectsEstimates(3);
%     interactionEffect = fixedEffectsEstimates(4);

%     SE_intercept = SEs(1);
%     SE_groupEffect = SEs(2);
%     SE_binEffect = SEs(3);
%     SE_interactionEffect = SEs(4);

%     % Perform pairwise comparisons within each bin
%     for b = 1:numBins
%         bin = binLevels{b};
%         binIndex = str2double(bin); % Convert bin label to numerical index if necessary
        
%         for i = 1:numGroups
%             % Calculate the estimated effect for each group within the current bin
%             if i == 1 % First group (e.g., DAO)
%                 estimateGroup1 = intercept + binEffect * binIndex;
%                 SE_Group1 = sqrt(SE_intercept^2 + (binIndex * SE_binEffect)^2);
%             elseif i == 2 % Second group (e.g., PO)
%                 estimateGroup2 = intercept + groupEffect + binEffect * binIndex + interactionEffect * binIndex;
%                 SE_Group2 = sqrt(SE_intercept^2 + SE_groupEffect^2 + (binIndex * SE_binEffect)^2 + (binIndex * SE_interactionEffect)^2);
%             end
%         end
