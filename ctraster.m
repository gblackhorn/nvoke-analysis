...
%	varargin{5} - post_stim_duration: calcium transients in this time duration after stimulation starts will be shown in plot 
	...
	row_interval = 0.4; % interval of rows in raster plot. distance between row (center to center) is 1 in total
	...
	peak_table_col_str = {'recording', 'neuron', 'stimu_num', 'peak_s_aligned',...
	 'peakTotal', 'prePeak', 'peakDuringStim', 'postPeak',...
	 'prePeakDpeakTotal', 'peakDuringStimDpeakTotal', 'postPeakDpeakTotal'};
	duration_pre_peak = 3; % time duration used to count peaks mentioned in 'peak_table_col_str' above
	duration_post_peak = 3;
	sort_col = 5; % 5-peakTotal, 6-prePeak, 7-peakDuringStim, 8-postPeak, 
				  % 9-prePeakDpeakTotal, 10-peakDuringStimDpeakTotal, 11-postPeakDpeakTotal
	    ...
	    for rn = 1:recording_num
	    	...
	    	for roi_n = 1:roi_num % number of roi
	    		...
	    		for gn = 1:gpio_train_num % number of stimulation train
	    			% next line organize peaks (as seconds) in roi row and gpio column
	    			rec(rn).peak{roi_n, gn} = peak_time(find(peak_time>=plot_start_time(gn) & peak_time<=plot_end_time(gn)));
	    			...
	    			peak_table_array{ptc, 2} = roi_n;
	    			peak_table_array{ptc, 3} = gn;
	    			peak_table_array{ptc, 4} = rec(rn).peak_gpio_align{roi_n, gn};
	    			peak_table_array{ptc, 5} = peak_num;
	    			peak_table_array{ptc, 6} = length(peak_data(find(peak_data>=0-duration_pre_peak & peak_data<=0))); % number of pre_peaks
	    			peak_table_array{ptc, 7} = length(peak_data(find(peak_data>=0 & peak_data<=gpio_duration))); % number of pre_peaks
	    			peak_table_array{ptc, 8} = length(peak_data(find(peak_data>=gpio_duration & peak_data<=gpio_duration+duration_post_peak))); % number of post_peaks
	    			peak_table_array{ptc, 9} = peak_table_array{ptc, 6}/peak_table_array{ptc, 5};
	    			peak_table_array{ptc, 10}= peak_table_array{ptc, 7}/peak_table_array{ptc, 5}; 
	    			peak_table_array{ptc, 11}= peak_table_array{ptc, 8}/peak_table_array{ptc, 5}; 
    ...
    % 	[roi_num, gpio_train_num] = size(rec(rn).peak);
    % 	for roi_n = 1:roi_num
    % 		for gn = 1:gpio_train_num
    ...
    % 			plot_x = rec(rn).peak_gpio_align{roi_n, gn}; 
    ...
    % 				line(plot_xx, plot_yy, 'Color', 'k', 'LineWidth', 3);
    % 			end
    % 			plot_row = plot_row+1;
    % 		end
    % 	end
    % end
   	...
   	for sn = 1:size(peak_table_array_sorted, 1) % stimulation number
	   	plot_x = peak_table_array_sorted{sn, 4};
	   	plot_xx(1:3:(length(plot_x)*3-2)) = plot_x;
	   	plot_xx(2:3:(length(plot_x)*3-1)) = plot_x;
	   	plot_xx(3:3:(length(plot_x)*3)) = NaN;
	   	plot_y_low = plot_row-(1-row_interval)/2; % low lim of each line
